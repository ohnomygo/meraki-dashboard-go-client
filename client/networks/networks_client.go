...
func (a *Client) GetNetworkPoliciesByClient(params *GetNetworkPoliciesByClientParams, authInfo runtime.ClientAuthInfoWriter, opts ...ClientOption) (*GetNetworkPoliciesByClientOK, error) {
	// TODO: Validate the params before sending
	if params == nil {
		params = NewGetNetworkPoliciesByClientParams()
	}
	op := &runtime.ClientOperation{
		ID:                 "getNetworkPoliciesByClient",
		Method:             "GET",
		PathPattern:        "/networks/{networkId}/policies/byClient",
		ProducesMediaTypes: []string{"application/json"},
		ConsumesMediaTypes: []string{"application/json"},
		Schemes:            []string{"https"},
		Params:             params,
		Reader:             &GetNetworkPoliciesByClientReader{formats: a.formats},
		AuthInfo:           authInfo,
		Context:            params.Context,
		Client:             params.HTTPClient,
	}
	...
	success, ok := result.(*GetNetworkPoliciesByClientOK)
	if ok {
		return success, nil
	}
	// unexpected success response
	// safeguard: normally, absent a default response, unknown success responses return an error above: so this is a codegen issue
	msg := fmt.Sprintf("unexpected success response for getNetworkPoliciesByClient: API contract not enforced by server. Client expected to get an error, but got: %T", result)
	panic(msg)
}
