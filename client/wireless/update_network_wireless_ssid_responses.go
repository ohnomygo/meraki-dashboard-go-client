...
type UpdateNetworkWirelessSsidBody struct {
	...
	// Enum: [8021x-google 8021x-localradius 8021x-meraki 8021x-nac 8021x-radius ipsk-with-radius ipsk-without-radius open open-enhanced open-with-nac open-with-radius psk]
...
func init() {
	var res []string
	if err := json.Unmarshal([]byte(`["8021x-google","8021x-localradius","8021x-meraki","8021x-nac","8021x-radius","ipsk-with-radius","ipsk-without-radius","open","open-enhanced","open-with-nac","open-with-radius","psk"]`), &res); err != nil {
		panic(err)
	}
	for _, v := range res {
